// assuming that the array will always have more than k
export function sortKSortedArray(array: number[], k: number) {
  const minHeap = new Heap([...array].slice(0, k + 1), MIN_HEAP_FUNC);
  let insertPosition = 0;

  for (let i = k + 1; i < array.length; i++) {
    // first we insert, then we add into the heap, until we have added all the numbers
    array[insertPosition] = minHeap.remove()!;
...
describe("sort k sorted array", () => {
  test("should work", () => {
    const array = [3, 2, 1, 5, 4, 7, 6, 5];
    console.log(sortKSortedArray(array, 3));
  });
});
