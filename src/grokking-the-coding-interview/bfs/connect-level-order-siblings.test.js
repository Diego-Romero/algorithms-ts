...
const connect_level_order_siblings = function (root) {
  ...
  while (queue.length) {
    ...
    // connect all the siblings to the next one, except the last
    for (let i = 0; i < siblings.length - 1; i++) {
      // iterate until one before last
      console.log("connecting", siblings[i].val, siblings[i + 1].val);
